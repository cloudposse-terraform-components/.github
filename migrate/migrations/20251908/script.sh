...
codex exec  --full-auto "We need to update README.yaml. There is a description that just contains a markdown of the whole text. The description have to be decomposed into several sections. So, Description, Usage, References, Categories. And the important thing is that we shouldn't lose any piece of information defined in the description. Use this https://github.com/cloudposse/.github/blob/main/README.md.gotmpl to get README.yaml structure. If referneces link does not have description - set it empty.  Run 'atmos readme' after updating README.yaml to verify the changes. Fix errors if any."
